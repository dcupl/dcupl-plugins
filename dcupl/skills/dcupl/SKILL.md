...
- **Large datasets can OOM the internal daemon, and `dcupl validate` does NOT expose `--max-memory` / `--quality`** (a known dcupl-cli#119 follow-up gap). Error tracking materializes one record per problem cell — a wide/sparse dataset is millions of records and the build is heavy; if it crashes (`FATAL ERROR: Reached heap limit`), fall back to the manual daemon: `dcupl app create --load --auto-serve --max-memory <MB> [--quality false] …` then query `$DcuplErrorTrackingErrors` (see `dcupl app create --help` / `references/querying.md`). `--quality false` gives a structural-only load (parses + counts every row, skips error tracking) — a clean count is then your validation signal. (See dcupl/dcupl#178.)
