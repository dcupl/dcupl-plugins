...
**Which CDN host `--source remote` talks to** (CLI 1.4.0-beta.4+, dcupl/dcupl-cli#139 — older CLIs always hit the prod CDN and ignore `consoleApiUrl`): `--cdn-base-url <url>` > `DCUPL_CDN_BASE_URL` env > `cdnBaseUrl` in `dcupl.config.json` > mapping from `consoleApiUrl` (`api.dcupl.com` → `cdn.dcupl.com`, `api.dev.dcupl.com` → `cdn.dev.dcupl.com`; the old `dev.api` → `dev-cdn` pair still works) > SDK default (prod). Develop hosts follow `<service>.dev.dcupl.com` since 2026-09-22; `cdn.dev.dcupl.com` counts as a dcupl CDN only with `@dcupl/loader` from dcupl/dcupl#264 onwards — older SDKs need `dev-cdn.dcupl.com`. A dev project needs one of these set or the load 404s. `LOADER_CONFIG_FETCH_FAILED` now names the attempted URL (`--json`: `details.url` / `details.source`, api key redacted) — read it before guessing at credentials. The same resolution applies to `app loaders add` and `validate`.
...
>
> ⚠️ **Treat curated examples as shape guidance, not gospel — when an example and the schema disagree, the schema wins.** Examples have historically drifted from the live API; if one shows a field or sandbox global that the schema (or a reference file here) doesn't, trust the schema.
...
The JSON shape: `{ ok, source, load:{ok,error?}, models:{total,empty[],autoGenerated[]}, errors:{ total, bySeverity:{critical,warning}, byGroup, byType, truncatedGroups[], records[] }, summary:{critical,warnings} }`.
...
- **`errors.records`** = capped at `--limit` (default 10); each names `model`, `attribute`, `errorGroup`, `errorType`, `title`. It is a *sample*, not the full list — `--json` does not lift the cap, raise `--limit` instead (and even then a group stores at most 1000 records, see below). For the full taxonomy see "Model & data errors" below.
- **`errors.total` / `bySeverity` / `byGroup` / `byType`** = **exact** counts on a CLI that includes dcupl/dcupl-cli#209: they are summed from `$DcuplErrorTrackingSummary`, so a group with 44k errors reads 44k, not 1000. **`errors.truncatedGroups`** lists every `(model, attribute, errorType)` group whose stored records are only a sample — `{ model, attribute, errorType, totalCount, storedCount }` — and the text output repeats them under "Truncated groups"; `[]` means nothing was capped. **Detect the behavior by the key, not the version:** if `errors.truncatedGroups` is *absent* from the JSON, the CLI predates #209 (or bundles an SDK older than 2.0.0-beta.17) and the counts are facets over the capped `$DcuplErrorTrackingErrors` — treat any group reading exactly 1000 as "1000 **or more**" (see the record cap under "Model & data errors").
...
- `dcupl generate <type>` scaffolds one of (exact choices): `model`, `script`, `transformer`, `operator`, `typescript`, `json-schema`, `test-setup`, `test`, `test-config`, `workflow`. (Note: it's `typescript`/`json-schema`, NOT `type`/`schema`. `generate workflow --name <key>` writes `workflows/<key>.workflow-v3.json` from a preset — currently only `read-transform-write`, selectable with `--template` — and prompts for the key if `--name` is omitted, so always pass it in agent contexts. `generate model --from <data file>` infers a model from CSV/JSON — see `references/model-authoring.md`.) **Caveat:** several `generate` subcommands are interactive and crash with a raw `ExitPromptError` stack trace in non-TTY/agent contexts (e.g. `generate test` has a prompt with no backing flag). But `generate model --from … --json` works well — it emits structured JSON including a `diagnostics[]` array (`inferredType`, `observedTypes`, `conflict`, `emptyRate` per column) and the chosen `keyStrategy` (it detects duplicate-key columns and falls back to `autoGenerateKey`); when scripting, read that JSON rather than re-parsing the file. `config get --json` also works (apiKey redacted). Only `config set` still ignores `--json` and prints human output.
...
When error tracking is enabled, the internal Model `$DcuplErrorTrackingErrors` collects every error a Model or its Data may have. When diagnosing model/data problems, query this Model to see which errors are actually present rather than guessing. Each record carries `type`, `errorGroup`, `errorType`, `title`, and (where relevant) `model`, `itemKey`, `attribute`, `description`, `meta` — plus `groupTotal` / `truncated` on SDKs 2.0.0-beta.17 or newer (see the record cap below).

> ⚠️ **Record cap.** The model stores at most **1000 records per error group** (`maxErrorsPerGroup`, an OOM guard). SDKs 2.0.0-beta.17 or newer (dcupl/dcupl#204) expose the **true totals without lifting the cap**: query the companion model `'$DcuplErrorTrackingSummary'` — one record per `(model, attribute, errorType)` group with `totalCount`, `storedCount` and `truncated` — and every stored `$DcuplErrorTrackingErrors` record carries `groupTotal` / `truncated` (facet or filter on `truncated` to spot capped groups). `dcupl validate` already does this for you — its totals and `errors.truncatedGroups` come from that summary (dcupl/dcupl-cli#209). For manual daemon queries: the daemon bundles its own SDK, so probe first: `dcupl app query execute --model '$DcuplErrorTrackingSummary'` returning `MODEL_NOT_FOUND` means an older SDK. On those, both `query execute` and `fn facets` against the error model present the capped number as if it were the total (44k identical errors facet as `count: 1000`): treat any group sitting at exactly 1000 as "1000 **or more**" and quantify the real extent with a direct probe on the affected model (e.g. `fn metadata --query '{"operator":"isTruthy","attribute":"<attr>","value":false}'`). See **Validate a workspace** above for the exact commands to load a workspace and query these Models.
