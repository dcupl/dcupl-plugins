...
> ⚠️ **Record cap.** The model stores at most **1000 records per error group** (`maxErrorsPerGroup`, an OOM guard) — and both `query execute` and `fn facets` against it present the capped number as if it were the total: a workspace with 44k identical errors facets as `count: 1000`. Treat any group sitting at exactly 1000 as "1000 **or more**" and quantify the real extent with a direct probe on the affected model (e.g. `fn metadata --query '{"operator":"isTruthy","attribute":"<attr>","value":false}'`). See **Validate a workspace** above for the exact commands to load a workspace and query this Model.
