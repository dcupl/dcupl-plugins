...
- **`errors.records`** = capped at `--limit` (default 10); each names `model`, `attribute`, `errorGroup`, `errorType`, `title`. Counts (`byGroup`/`byType`) are facets over `$DcuplErrorTrackingErrors`, so until dcupl/dcupl-cli#209 lands they are subject to the 1000-per-group record cap (see "Model & data errors" below — a group reading exactly 1000 may be larger); `records[]` is additionally capped by `--limit`. For the full taxonomy see "Model & data errors" below.
